...
function expiration() {
    ...
    let expiration = new Date(currentDate.getTime() + 10 * 60000); // 10 minutos de expiração
    return expiration
}
...
async function init (req, res) {
    ...
    try {
        ...
        let paymentBody = {
            ...
            // Removida notification_url e metadata, pois o frontend fará o polling
        };
        ...
        if (req.body.paymentType === "card") {
            ...
            if (req.body.issuerId) {
                paymentBody.issuer_id = Number(req.body.issuerId);
            }
        ...
        console.log("Dados do pagamento enviados ao Mercado Pago:", JSON.stringify(paymentBody, null, 2));
        let result = await payment.create({
            body: paymentBody
        });
        console.log("Resultado do pagamento do Mercado Pago:", JSON.stringify(result, null, 2));
        res.json({ result });
    } catch (error) {
